...
def test_function_call_without_args():
    # Call with one arg
    tok_stream = [Token(TokType.ID, "hello"), Token(TokType.LROUND), Token(TokType.RROUND), Token(TokType.EOF)]
    p = Parser(tok_stream, "")
    call_node = p.access()
...
def test_call_missing_begin_bracket():
    tok_stream = [Token(TokType.ID, "hello"), Token(TokType.LROUND), Token(TokType.EOF)]
    p = Parser(tok_stream, "")
    with pytest.raises(InternalParseError):
        call_node = p.access()
...
#     tok_stream = [Token(TokType.ID, "hello"), Token(TokType.RROUND), Token(TokType.EOF)]
#     p = Parser(tok_stream, "")
#     with pytest.raises(InternalParseError):
#         call_node = p.access()
